...
class ListFilesBack : public Route {
public:
    ListFilesBack(const std::string &basePath, const std::string &urlBase = "", const std::string & path = ".*") : Route(
            path) {
    ...
    bool Run(Request &request, Response &response, Socket &socket) override {
        ...
        if (normalPath.size() > 0 && pppath.size() < path.size())
            rs << "<a href=\"" << "javascript:window.history.back()" << "\">" << ".." << "</a><br>";
